<template>
    <div>
        <div class="form-el-content form-select">
            <div @click.stop="dropdownShow = !dropdownShow">
                <input :readonly="dataKey.hasManual !== true" type="text" class="text form-input" :class="dataKey.css" v-model="selectLabel"
            ...
            <transition>
                <ul class="select-dropdown" v-if="dropdownShow && !dataKey.disabled">
                    <template v-for="item in dataKey.sortArray">
                        <li :value="item.value" class="select-li" :class="{'active': dataKey.key == item.value, 'disabled': item.disabled}" @click.stop="changeSelect(item.value, item.title)">{{item.title}}</li>
                    </template>
                        <li v-if="dataKey.hasManual" class="select-li" @click.stop="hanlderManual()">{{dataKey.manualText}}</li>
                </ul>
            </transition>
    ...
    methods: {
        ...
        setInputValue() {
            ...
            this.dataKey.sortArray.forEach(function(item) {
                //当值存在于下拉列表时
                if (_this.dataKey.key == item.value) {
                    newVal = item.title;
                }
            });

            if(!newVal) {
                newVal = this.dataKey.key;
            }

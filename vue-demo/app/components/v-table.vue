<template>
    <div class="form-group" v-show="tableOptions.show" :class="tableOptions.css">
        ...
        <table class="table table-fixed">
            <thead>
                <tr>
                    <th :width="columns.width" v-for="columns in tableOptions.columns" :key="columns">
                        <span
            ...
            <table class="table table-fixed" ref="table-body">
                <tbody>
                    <tr
                        ...
                        :data-key="rowsData[tableOptions.key]"
                        :key="rowsData"
                    >
    ...
    methods: {
        ...
        goSearch() {
            if (this.searchValue == "") {
                ...
                this.tableData = filterTable(this.originData, this.searchValue);
            }
        ...
        updateTable() {
            ...
            this.$nextTick(function() {
                let parentHeight =
                        _this.$refs["table-body"].parentNode.offsetHeight,
                    tableBodyHeight = _this.$refs["table-body"].offsetHeight;
                if (parentHeight < tableBodyHeight) {
                    _this.tableScroll = true;
                } else {
                    _this.tableScroll = false;
                }
                _this.tabaleCallback(); //执行表格更新的回调
            });
        },
...
.table-search {
    ...
    .icon-search {
        ...
        line-height: $form-line-height - 2;
        height: $form-line-height - 2;
